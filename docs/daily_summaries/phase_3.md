...
4. Port `blkest`, `cpblock`, `linbin`, `linbin2D`, `rlbin` (internal helper functions calling `.Fortran()`) as thin wrappers over `py-KernSmooth._KernSmooth` Fortran subroutines, with `np.asarray(..., dtype=np.float64)` / `np.zeros(..., dtype=np.int32)` replacing `as.double` / `as.integer` / `double` / `integer` buffer allocations.
