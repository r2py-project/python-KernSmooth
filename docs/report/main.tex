...
The working directory \texttt{python-KernSmooth/} is the working tree of the \texttt{python-KernSmooth} repository hosted at \texttt{github.com/r2py-project/python-KernSmooth}. The \texttt{r2py\_kernsmooth/} subdirectory is additionally managed as a git subtree linked to a separate repository at \texttt{github.com/r2py-project/r2py\_kernsmooth}, allowing the Python package to be versioned and published to PyPI independently of the broader project. The cluster scripts \texttt{git\_pull.sh} and \texttt{git\_push.sh} at the project root encapsulate the subtree synchronisation commands required to keep both remotes up to date from the HPC node.
...
\texttt{r2py\_kernsmooth/} &
  Git subtree linked to \texttt{github.com/r2py-project/r2py\_kernsmooth}; the installable Python package. Internal structure detailed in the table below. \\
...
The result was written to \texttt{language\_dependency\_analysis/R/all.csv}: a 509-row, 4-column CSV (\texttt{language\_dependency}, \texttt{function\_name}, \texttt{line\_number}, \texttt{call\_body}) covering all 16 functions in \texttt{all.R}.
...
\texttt{dpill}   & 38 & 17 & 28 &  83 & 21 \\
\texttt{locpoly} & 38 & 11 & 23 &  72 & 72 \\
\midrule
\textbf{Total}   & \textbf{240} & \textbf{96} & \textbf{178} & \textbf{514} & \textbf{452} \\
\bottomrule
\end{tabular}
\end{center}
