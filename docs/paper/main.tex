...
\fancyhead[L]{\small\itshape AI-Assisted R-to-Python Package Conversion}
\fancyhead[R]{\small\itshape Cai}
...
We address these limitations with a seven-phase methodology that decomposes the translation problem into specialised, ordered steps, each executed by a dedicated AI skill that orchestrates one or more sub-agents. The methodology generates a machine-readable catalogue of all language dependencies and their translation nuances \textit{before} any code is written, ensuring consistency across all conversion agents. Functions are converted in topological dependency order, so each conversion agent can inspect the Python API of already-converted callees. The converted package is validated against the live R reference implementation using \texttt{rpy2} at a specified numerical tolerance, and an iterative automated bug-resolution loop continues until all tests pass. The resulting framework is designed to be reproducible, auditable, and extensible to packages of greater complexity.

We demonstrate the methodology on \pkg{KernSmooth}~\cite{wandjones1995}, a recommended R package implementing kernel density estimation, plug-in bandwidth selection, and local polynomial regression. \pkg{KernSmooth} serves as an ideal proof-of-concept: it has zero runtime dependencies on contributed CRAN packages (eliminating the recursive dependency problem), its computationally intensive work is fully delegated to compiled Fortran~77 routines (testing the methodology's ability to handle the R Fortran foreign-function interface), and its seven public functions implement well-defined algorithms with exact numerical specifications, enabling rigorous validation. The framework is described at a level of generality intended for application beyond this specific example.
...
The methodology is structured as a pipeline of seven phases (Fig.~\ref{fig:pipeline}). Each phase is motivated by a distinct risk of error or inconsistency in the translation process; the phase is resolved by invoking one or more \textit{skills}---top-level slash commands encoded as structured natural-language specifications stored under \texttt{.claude/commands/}---which orchestrate one or more \textit{sub-agents}---specialised worker agents whose specifications reside under \texttt{.claude/agents/} (see \suppnote{1} for the architecture of skills and agents; full specifications are given in \suppnote{2} and \suppnote{3}). At the conclusion of each phase, the \skill{summarize-research-report} skill produced a structured phase summary saved to \texttt{docs/daily\_summaries/}, providing a durable record of decisions made.

\subsection{Phase 1: Static Structural Analysis}
\label{sec:phase1}

Before any translation work can begin, a precise machine-readable map of the source package's internal structure is required. Without it, the conversion agent for each function must infer architectural facts independently, risking inconsistent dependency classification and missed foreign-function interface calls. We therefore began by analysing the source package statically.
...
The \skill{analyze-r-folder-dependencies} skill (\suppnote{3}) was invoked with the R source directory as its argument. The skill scans the target folder for \texttt{.R} files and dispatches the \agent{analyze-r-file-dependencies} sub-agent (\suppnote{2}) once per file, sequentially. Because \pkg{KernSmooth} consolidates all R code into a single file (\texttt{KernSmooth/R/all.R}), exactly one agent invocation was required.

The sub-agent parsed the R source and classified every function call within each user-defined function into three mutually exclusive categories: (i)~\textit{language dependencies}, calls to R built-in or standard-library functions; (ii)~\textit{internal dependencies}, calls to other functions defined within the same R file; and (iii)~\textit{external dependencies}, calls through the Fortran or C foreign-function interface (\texttt{.Fortran()}, \texttt{.C()}, \texttt{.Call()}, or \texttt{.External()}). The output is a JSON object keyed by function name. For \pkg{KernSmooth}, the result documented 16 functions, 5 distinct language dependencies (\texttt{dbeta}, \texttt{dnorm}, \texttt{fft}, \texttt{quantile}, \texttt{var}), an internal call graph with three dependency levels, and eight distinct Fortran entry points---a prerequisite for all subsequent phases.

The structural analysis output was then synthesised with the package's \texttt{DESCRIPTION}, \texttt{NAMESPACE}, and all seven \texttt{.Rd} documentation files into a comprehensive architecture document (\texttt{docs/architecture-analysis.md}), providing a single human-readable reference consulted throughout all later phases. This synthesis step identified that \pkg{KernSmooth} has no runtime CRAN dependencies (only \texttt{stats}-package built-ins are called), substantially simplifying the translation scope.

\subsection{Phase 2: Python Build Infrastructure}
\label{sec:phase2}

The conversion strategy retains the original Fortran~77 source files verbatim and compiles them into a Python C-extension via NumPy's f2py tool~\cite{peterson2009f2py}, isolating the risk of numerical error to the R-wrapper translation layer. Phase~2 established the packaging infrastructure necessary for both local development and broad distribution before any wrapper code was written.

This phase did not involve a skill invocation; the build system was designed manually based on the architectural requirements identified in Phase~1. The \texttt{meson-python} backend was selected as the build system because it natively supports f2py targets and produces PEP~517-compliant wheels; details of the build system design, BLAS discovery logic, and PyPI distribution configuration are described in the Methods and \suppnote{5}. The key design principle is that numerical differences between the Python and R packages can originate only in the R-to-Python wrapper translation, not in the underlying Fortran routines, providing a strong correctness guarantee.

\subsection{Phase 3: Language Dependency Catalogue and Translation Guide Generation}
\label{sec:phase3}

The central risk of any LLM-assisted code translation is the inconsistent handling of language-specific semantic differences across function conversion steps. This risk is especially acute for statistical software: an LLM operating on a single function may correctly handle the FFT normalisation convention in one instance but fail to do so in another, producing a package where the same underlying discrepancy is treated differently in different functions. To address this, Phase~3 systematically catalogued every R standard-library call in the source package and generated a dedicated, machine-readable translation guide for each distinct dependency before any conversion began.
...
Two skills were deployed in this phase (Fig.~\ref{fig:phase3}). First, the \skill{extract-r-folder-language-dependencies} skill (\suppnote{3}) was invoked with the R source directory and the Phase~1 structural analysis JSON directory as arguments. The skill dispatched the \agent{extract-r-file-language-dependencies} sub-agent (\suppnote{2}) once per R file together with its structural analysis JSON, which pre-classifies each function's language dependencies, eliminating the need for the agent to infer dependency types from scratch. The agent rescanned the R source line by line and recorded, for every individual call site, the dependency name, the enclosing function name, the line number, and the complete call expression (including multi-line calls). The result was a 370-row, 4-column CSV covering all 16 functions in \texttt{all.R}. A utility script merged per-file CSVs into a combined table sorted by dependency name, making all call sites for each dependency contiguous for subsequent processing.

Second, the \skill{generate-language-dependency-conversion-guides} skill (\suppnote{3}) was invoked with the R source directory and the combined CSV as arguments. The skill identified 46 unique dependency names in the CSV and launched 46 instances of the \agent{generate-language-dependency-conversion-guide} sub-agent (\suppnote{2}) in a \textit{single parallel batch}. Parallel dispatch is appropriate here because the translation guide for each dependency is logically independent of all other guides; there is no ordering constraint, and parallelism reduces wall-clock time from $O(N)$ to $O(1)$ agent invocations. Each agent received its assigned dependency's CSV subset, read the corresponding source lines in \texttt{all.R} for contextual understanding, and optionally consulted the R online documentation for complex or non-obvious behaviors. The agent then produced a structured Markdown guide specifying the correct Python translation strategy with concrete code examples.

The 46 resulting guides were saved to \texttt{language\_dependency\_analysis/conversion\_guides/}. The most consequential guides---those where an incorrect translation would produce a silent numerical error with no runtime exception---are described in the Methods. Although no manual correction of the guides was performed, their structured format makes targeted human review and correction straightforward, providing a control point that constrains the risk of errors propagating into Phase~4. This is a key advantage over an unguided single-pass LLM translation: the guides make every language-semantic decision explicit, auditable, and consistent.

\subsection{Phase 4: Automated Function Conversion and Package Assembly}
\label{sec:phase4}

With the dependency map (Phase~1), build infrastructure (Phase~2), and translation guides (Phase~3) in place, Phase~4 executed the actual function conversion. Two challenges make this non-trivial despite the preparation.
...
First, functions must be converted in dependency order: a callee's Python API must exist before the caller's conversion agent can write correct call expressions. Second, the Fortran foreign-function interface in Python (via f2py) differs fundamentally from R's \texttt{.Fortran()} in how it handles output arguments (see Methods), and this difference is not visible from the R source, requiring targeted awareness in conversion agents.

The \skill{convert-r-file-to-python} skill (\suppnote{3}) was invoked with the R source file, the structural analysis JSON, the dependency-level CSV, the conversion guide folder, and the output folder as arguments (Fig.~\ref{fig:phase4}). The skill parsed the dependency-level CSV to derive a topological ordering of the 16 functions, from deepest leaves (Level~2) to roots (Level~0), and dispatched the \agent{convert-r-function-to-python} sub-agent (\suppnote{2}) \textit{sequentially} once per function in that order. Sequential dispatch here is not a performance limitation but a correctness requirement: each agent reads the JSON artefacts of the function's internal callees to understand their Python signatures and calling conventions, and those artefacts must already exist when the agent is invoked. Each agent received the R source fragment for its function, the function's JSON dependency map from the structural analysis, the folder of 46 conversion guides, and the output folder for the resulting JSON artefact. The agent produced a self-contained JSON encoding of the converted Python function, including imports, type annotations, and docstring. The 16 resulting JSON artefacts were written to \texttt{conversion\_results/R/all.R/}.

Following conversion, the \skill{combine-python-functions-into-file} skill (\suppnote{3}) assembled the 16 JSON artefacts into the final \texttt{r2py\_kernsmooth/\_\_init\_\_.py}. Before writing, the skill scanned the \texttt{meson.build} and the existing module skeleton to verify the correct relative import path for the compiled Fortran extension, then corrected three categories of systematic errors present in the per-function JSON artefacts: absolute import paths were replaced with relative imports (\texttt{from~.~import~\_KernSmooth}); redundant cross-function imports were removed (functions sharing a module need not import each other); and duplicate third-party import statements were merged. The final import block was sorted in PEP~8 standard-library $\to$ third-party $\to$ local order. Syntax correctness was confirmed immediately by \texttt{ast.parse()}. After Phase~4, the package was in a runnable state with all 16 functions converted.

\subsection{Phase 5: Code Quality Audit and Namespace Refinement}
\label{sec:phase5}

The automated conversion pipeline prioritises functional correctness. Phase~5 was a systematic audit and refactoring pass addressing three categories of concern not directly handled by the conversion agents: type annotation completeness, namespace hygiene, and residual code-quality defects from the porting process.

This phase did not invoke a new skill; it was a manual review of the assembled \texttt{\_\_init\_\_.py} guided by comparisons against the R source. The audit identified and resolved: (i)~bare container-type annotations lacking element type parameters (corrected to fully parameterised forms, e.g.\ \texttt{np.ndarray[Any, np.dtype[np.float64]]}); (ii)~a silent crash bug in three public functions where a \texttt{None}-typed optional parameter was unconditionally passed to \texttt{np.asarray()}, producing a degenerate array that caused a \texttt{TypeError} at the subsequent \texttt{len()} call; (iii)~a 27-line bandwidth-discretization block duplicated identically across three functions, extracted into a private helper \texttt{\_discretize\_bandwidth()}; (iv)~an 8-line partial-string-match block duplicated across four call sites, extracted into \texttt{\_resolve\_choice()}; and (v)~dead code consisting of R lifecycle hooks (\texttt{.onAttach}/\texttt{.onUnload} transliterations) that are unreachable in Python's import machinery. Every error and warning message in the Python module was also verified against the corresponding \texttt{stop()} and \texttt{warning()} calls in the R source, and three discrepancies in message format or conditions were corrected. Details of the specific corrections are given in the Methods.

\subsection{Phase 6: R Test Conversion and Validation Infrastructure}
\label{sec:phase6}

A validated test suite grounded in the R reference implementation is the primary mechanism for detecting semantic errors in the converted package. Phase~6 ported the two regression tests shipped with the R \pkg{KernSmooth} package into Python equivalents and upgraded them to assertion-based numerical comparisons using rpy2~\cite{gautier2008rpy2}, which calls the R runtime at test time from Python.

The \skill{convert-r-tests-to-python} skill (\suppnote{3}) was invoked with the R test folder, the R package directory, the Python package directory, and the output test folder as arguments. The skill scanned the R test folder for \texttt{.R} scripts and dispatched the \agent{convert-r-test-to-python} sub-agent (\suppnote{2}) once per file, sequentially, producing an initial Python script for each. After the f2py compatibility issues described in the Methods were resolved, the skill was re-invoked and produced fully assertion-based \texttt{pytest} files.

The resulting tests use rpy2 throughout: R packages are loaded once at module level with \texttt{importr()}; input data is passed as \texttt{ro.FloatVector} or \texttt{ro.IntVector}; results are extracted with \texttt{.rx2()} and converted to NumPy arrays; and numerical agreement is asserted with \texttt{numpy.testing.assert\_allclose} at explicitly specified tolerances (\texttt{rtol=1e-10} for grid coordinates, \texttt{rtol=1e-6} for density and regression estimates). This rpy2-based testing pattern---rather than hardcoded reference values---was adopted deliberately: hardcoded values require manual regeneration when the R reference changes and may introduce spurious failures due to platform-specific floating-point differences, whereas rpy2-based tests always compare against the current R implementation. All four tests passed after the f2py fixes.
...
At the conclusion of Phase~6, the test suite comprised four tests covering only three of the seven public functions. Phase~7 systematically built comprehensive positive, negative, and edge-case coverage for all seven public functions and employed an iterative automated bug-resolution loop to bring the full suite to zero failures.

The \skill{generate-python-file-tests} skill (\suppnote{3}) was invoked with the Python module as its argument. The skill parsed \texttt{\_\_init\_\_.py} to enumerate all functions and identified the seven user-facing public functions by consulting the KernSmooth CRAN reference manual; the remaining internal utilities with no R documentation were excluded. The \agent{generate-python-function-tests} sub-agent (\suppnote{2}) was then dispatched \textit{sequentially} once per public function. Sequential dispatch is preferable here to avoid redundant rpy2 initialisation overhead and to allow later agents to avoid replicating edge cases already covered by earlier ones. Each agent parsed the function signature and docstring, fetched the corresponding R documentation page, and generated three test files: positive cases (\texttt{test\_\textit{fn}\_positive.py}), negative cases (\texttt{test\_\textit{fn}\_negative.py}), and edge cases (\texttt{test\_\textit{fn}\_edge.py}). All tests use rpy2 to obtain live R reference values. For negative tests, the agent applied a principled strategy for cases where R and Python raise errors with different messages or where one language raises while the other returns a non-finite result: such cases are documented by a \texttt{UserWarning} rather than treated as test failures, providing an honest record of behavioural divergences. 516 tests were generated across the seven functions (Table~1; Fig.~\ref{fig:tests}); 454 passed on the first run, with 62 failures concentrated entirely in \texttt{dpill}.
...
The \skill{fix-bugs-found-in-tests} skill (\suppnote{3}) was invoked to resolve the 62 \texttt{dpill} failures. The skill follows an iterative protocol: it runs the full test suite with \texttt{pytest}, isolates the first failing test, dispatches the \agent{fix-bug-found-in-test} sub-agent (\suppnote{2}) with the failing test's traceback and the relevant source context, reinstalls the package if the fix modifies library code, and repeats until all tests pass. This iterative dispatch mode handles bugs that only become visible after prior bugs are fixed, which is precisely the case here: the three bugs in \texttt{dpill} formed a cascade where each prior bug masked the subsequent one (see Methods for details). Three sub-agent invocations were sufficient to resolve all failures. After the final package rebuild, running the complete suite with \texttt{python~-m~pytest~r2py\_kernsmooth/tests/~-q} produced 518~passed, 0~failed in 6.40~seconds under Python~3.14.4 / pytest~9.0.3. All seven public functions achieve numerical agreement with R to 6--10 significant figures across all positive and edge test cases.
