...
\fancyhead[R]{\small\itshape A Methodological Framework}
...
The R statistical computing environment hosts a large body of validated, high-performance statistical package implementations that are unavailable as native Python libraries. Converting these packages manually is time-consuming, error-prone, and fails to scale, while runtime bridging solutions such as rpy2~\cite{gautier2008rpy2} and reticulate~\cite{ushey2023reticulate} require an R installation and introduce inter-process overhead. Large language models (LLMs) offer a path towards automation~\cite{chen2021codex,roziere2020transcoder,wang2021codet5}, but unguided translation of statistical code introduces silent numerical errors arising from semantic differences between R and Python that are non-obvious from source inspection alone. Here we present a seven-phase AI-assisted methodology for the systematic, reproducible conversion of R packages to Python, implemented using a structured hierarchy of orchestrating skills (top-level slash commands) and specialized sub-agents within the Claude Code AI development environment. The methodology systematically addresses each source of translation error: it catalogues every language-specific call site and generates dedicated machine-readable translation guides before any code is written; it converts functions in topological dependency order; and it validates output numerically against the live R implementation at a specified tolerance. We demonstrate the framework on \pkg{KernSmooth}~\cite{wandjones1995} (v.\,2.23-26), a recommended R package implementing kernel smoothing methods~\cite{sheatherjones1991,ruppertsheatherwand1995,wand1994}. The resulting Python package, \pkg{r2py\_kernsmooth}, passes 518 assertion tests against the R reference under Python~3.14, achieving agreement to 6--10 significant figures across all seven public functions.
...
We demonstrate the methodology on \pkg{KernSmooth}~\cite{wandjones1995}, a recommended R package implementing kernel density estimation, plug-in bandwidth selection, and local polynomial regression, producing the Python package \pkg{r2py\_kernsmooth}. \pkg{KernSmooth} serves as an ideal proof-of-concept: it has zero runtime dependencies on contributed CRAN packages (eliminating the recursive dependency problem), its computationally intensive work is fully delegated to compiled Fortran~77 routines (testing the methodology's ability to handle the R Fortran foreign-function interface), and its seven public functions implement well-defined algorithms with exact numerical specifications, enabling rigorous validation.

This project employs \textbf{Claude Code} (Anthropic) as the AI-assisted development environment, as it provides a structured agentic interface with native support for multi-agent orchestration, file-system access, and shell execution within a single session, and has emerged as the industry-standard platform for natural-language-directed software development. The framework is described at a level of generality intended for application beyond this specific example. All skills and agents are implemented as structured natural-language specifications stored in the \texttt{.claude/} directory, making them transparent, auditable, and reusable for future projects.
...
The agent produces a JSON artefact that maps each function name to its three types of dependencies. All functions in the same R source file are included in a single JSON file, which contains a JSON object keyed by function name. An example of the JSON files is shown in Fig.~\ref{fig:json_example}. The JSON files are then consumed by subsequent agents for language dependency extraction (Phase~3) and function conversion (Phase~4).
...
One may argue that these discrepancies are well-known and can be handled on a case-by-case basis during the conversion process in Phase~4. However, due to the stochastic nature of LLM inference, we cannot guarantee that all language dependencies will be correctly identified and translated whenever a function is converted. If there is no unified, machine-readable reference of all language dependencies and their translation nuances, then the risk of silent numerical errors or unexpected bugs in the final Python package is unacceptably high. Even if the translation is mostly correct, inconsistent choices of idioms or styles across functions can make debugging significantly more difficult. Furthermore, there are many language dependencies in R that have so many variations in their usage that it is hard to guarantee a correct translation without a comprehensive reference. For example, the \texttt{as.double()} function is used in many different contexts (such as on scalars, vectors, matrices, or even data frames), and its translation to Python can vary depending on the context. If we rely directly on the LLM to identify and translate each usage correctly during the conversion process, we may miss some cases or introduce errors. By contrast, if we systematically extract all language dependencies and their call sites in this phase, and then generate detailed conversion guides for each scenario for each dependency, we can ensure that the translation in Phase~4 is consistent and accurate across all functions, producing elegant and clean implementations that improve the readability and maintainability of the underlying mathematical algorithms.

As a consequence, Phase~3 systematically catalogues every R standard-library call in the source package and generates a dedicated, machine-readable translation guide for each distinct dependency before any conversion begins.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/phase_3.pdf}
...
Determining the appropriate translation unit is a non-trivial design decision. Since most R packages use functional programming paradigms (although object-oriented programming exists in R, it is less prevalent in practice), there are two choices for the basic translation block: functions or entire files. During Phase~1 and Phase~3, the independent sub-agents always process entire R files at once, since entire files are the natural unit of input and output, and the per-agent context remains manageable. Now, to convert the entire R package into Python, the agents must receive not only the original R code, but also the dependency maps (the JSONs and CSV from Phase~1), the generated conversion guides in Markdown (from Phase~3), the R and Python online language documentation, and the previously converted Python functions as a reference for writing correct call expressions to internal callees. This substantial context burden degrades translation quality when an entire file is processed at once. Translating one function at a time reduces the context per agent invocation, allowing each agent to focus on a single, well-defined translation unit without irrelevant surrounding code. As a result, functions are chosen as the basic translation block for the agents.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/phase_4.pdf}
...
\end{itemize}
This comprehensive coverage strategy is designed to maximize confidence in the numerical correctness and robustness of error handling of the converted package. As \pkg{r2py\_kernsmooth} is a numerical statistics package, more complex software-engineering concerns such as concurrency or stateful behaviour fall outside the scope of the test suite.

The \skill{generate-python-file-tests} skill (\suppnote{3}) is invoked with the Python module as its argument. The skill parses \texttt{\_\_init\_\_.py} to enumerate all functions and identifies the seven user-facing public functions by consulting the KernSmooth CRAN reference manual; the remaining internal utilities with no R documentation were excluded. The \agent{generate-python-function-tests} sub-agent (\suppnote{2}) is then dispatched \textit{sequentially} once per public function. Sequential dispatch is preferable here to avoid redundant \pkg{rpy2} initialisation overhead and to allow later agents to avoid replicating edge cases already covered by earlier ones. Each agent parses the function signature and docstring, fetches the corresponding R documentation page to gain a thorough understanding of the interrelationships between all parameters and return values, and generates three test files: positive cases (\texttt{test\_\textit{fn}\_positive.py}), negative cases (\texttt{test\_\textit{fn}\_negative.py}), and edge cases (\texttt{test\_\textit{fn}\_edge.py}). All tests use \pkg{rpy2} to obtain and catch live R reference values and exceptions.

For tests that are expected to throw exceptions (all negative tests and some edge cases), the agent applies a principled strategy. If both R and Python throw an error, the test is considered passed. If R throws an error but Python does not, the test is considered failed, and the agent automatically investigates the discrepancy and determines whether the Python implementation needs to be updated to match R's behaviour or whether the test case is not applicable due to language differences. For cases where one language raises while the other returns a non-finite result, or where R and Python raise errors with different messages (most of these cases are due to differences between language features, such as lazy evaluation vs. eager evaluation), such cases are documented by a \texttt{UserWarning} rather than treated as test failures, providing an honest record of behavioural divergences. Under those circumstances, the tests are written in a way determined by the agent itself to be informative about the language differences and the expected behaviour in each language. The agents will automatically run the tests and see whether the behaviors match the expectations. When they find failures because the tests are poorly designed (e.g., due to too tight precision tolerances), they are going to fix them until success (e.g., by adjusting tolerances or modifying test cases). 514 tests were generated across the seven functions (Table~\ref{tab:tests}; Fig.~\ref{fig:tests}); 452 passed on the first run, with 62 failures concentrated entirely in \texttt{dpill}.
...
The converted Python package \pkg{r2py\_kernsmooth} uses \texttt{meson-python} as its PEP~517 build backend, with the full build definition in \texttt{r2py\_kernsmooth/meson.build}. The \pkg{f2py}~\cite{peterson2009f2py} wrapper is generated by NumPy (v.\,2.4.3) as a Meson custom target: \texttt{python~-m~numpy.f2py~\$\{sources\}~-m~\_KernSmooth~--lower}. The \texttt{--lower} flag lowercases Fortran symbol names to match GFortran conventions. An initial build template incorrectly listed the generated wrapper file as \texttt{\_KernSmooth-f2pywrappers2.f90} (the Fortran~90 free-form variant); because all sources are Fortran~77 fixed-form, \pkg{f2py} generates \texttt{\_KernSmooth-f2pywrappers.f} (the fixed-form variant), and the \texttt{meson.build} \texttt{outputs} list was corrected accordingly.
...
\subsection{\pkg{f2py} Interface Semantics and Compatibility}

The most structurally significant difference between the R and Python Fortran FFI layers is in the handling of output arguments. R's \texttt{.Fortran()} always returns a named list of all arguments, including arrays modified in-place by the Fortran subroutine; the R idiom \texttt{.Fortran(F\_linbin,~\ldots)\$gcnts} extracts the modified output array from that list. An \pkg{f2py}-wrapped Fortran subroutine, by contrast, returns \texttt{None} unconditionally; output array arguments are modified in-place via C pointer, and the Python caller reads results by examining the pre-allocated arrays passed in. Scalar output arguments require special treatment: a Python \texttt{np.float64} scalar is immutable, so passing one does not allow Fortran to write back; a length-1 NumPy array (\texttt{np.zeros(1, dtype=np.float64)}) is mutable and passes a writable pointer. See \suppfig{2} for a schematic comparison of the two calling conventions.

A second compatibility issue specific to NumPy~2.4.3 is the absorption of Fortran dimension scalars. When \pkg{f2py} processes a subroutine with parameters such as \texttt{ss(M,~ippp)}, it infers the dimensions \texttt{M} and \texttt{ippp} from the shapes of the passed arrays at call time, removing them from the required positional argument list and making them optional with defaults inferred from array shapes. The Phase~4 conversion agents were not aware of this behaviour and retained the dimension integers at their original Fortran-order positions, causing all subsequent arguments to be misaligned. For the most complex affected subroutine (\texttt{locpol}), this reduced the call from 19 to 16 positional arguments after the correction.

A third compatibility issue affected the \texttt{blkest} subroutine's scalar output arguments (\texttt{sigsqe}, \texttt{th22e}, \texttt{th24e}), which \pkg{f2py} declared as \texttt{input float} (passed by value) rather than as writeable output arguments, because they appear as plain Fortran \texttt{DOUBLE PRECISION} scalars rather than as array arguments. The fix required generating a full f2py signature file (\texttt{r2py\_kernsmooth/src/\_KernSmooth.pyf}) from the Fortran sources and adding explicit \texttt{intent(out)} annotations to the three scalars in the \texttt{blkest} block. The \texttt{meson.build} custom target was updated to pass the \texttt{.pyf} file as the sole \pkg{f2py} input, causing \pkg{f2py} to honour the explicit intent declarations. After this fix, \texttt{blkest} returns a Python tuple of three floats rather than receiving pre-allocated length-1 arrays.
...
All 518 tests use \pkg{rpy2}~\cite{gautier2008rpy2} to obtain reference values from the live R \pkg{KernSmooth} package (v.\,2.23-26). R packages are loaded once at module level with \texttt{importr()}. Input vectors are passed as \texttt{ro.FloatVector} or \texttt{ro.IntVector}; named arguments are mapped using \texttt{rpy2}'s keyword convention. Results are extracted with \texttt{.rx2()} and converted to NumPy arrays via \texttt{np.array()}. Numerical agreement is asserted with \texttt{numpy.testing.assert\_allclose} at \texttt{rtol=1e-10} for grid coordinates and \texttt{rtol=1e-6} for density and regression estimates, reflecting the precision of the double-precision Fortran computations. Behavioural divergences (cases where R and Python raise different exceptions, or where one returns \texttt{NaN}/\texttt{Inf} while the other raises) are documented by \texttt{UserWarning} rather than assertion failure. For the \texttt{locpoly} tests, the Prestige dataset is extracted directly from R via \texttt{ro.r("Prestige\$income")} to guarantee exact row correspondence with the R reference. The complete per-function test content is described in \suppnote{7}.

\section*{Declarations}

\subsection*{Data and code availability}

The Python package \pkg{r2py\_kernsmooth} and all project artefacts (structural analysis JSONs, language dependency CSVs, conversion guides, converted Python module, test suite) are available at \url{https://github.com/r2py-project/python-KernSmooth}. The Python package is additionally available as a standalone repository at \url{https://github.com/r2py-project/r2py\_kernsmooth}. The original R \pkg{KernSmooth} package (v.\,2.23-26) is available from CRAN under an Unlimited license.
...
The author thanks the HPC facility of the University of Notre Dame for providing computational resources. The AI-assisted development was performed using Claude Code (Anthropic).
