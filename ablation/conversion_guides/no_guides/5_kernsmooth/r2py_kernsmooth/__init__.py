...
def bkde(x: np.ndarray[Any, np.dtype[np.float64]], kernel: str = "normal", canonical: bool = False, bandwidth: float | None = None, gridsize: int = 401, range_x: tuple[float, float] | list[float] | np.ndarray[Any, np.dtype[np.float64]] | None = None, truncate: bool = True) -> dict[str, np.ndarray[Any, np.dtype[np.float64]]]:
    ...
    if kernel not in allowed_kernels:
        raise ValueError("'kernel' should be one of " + str(allowed_kernels))
    ...
    if kernel == "normal":
        z = lvec * delta
        kappa = (np.exp(-z ** 2 / 2) / np.sqrt(2 * np.pi)) / (n * h)
    elif kernel == "box":
    ...
    ## P * np.fft.ifft(z). The R code divides by P afterwards, which exactly
    ## cancels numpy's normalization, so np.fft.ifft(z) can be used directly.
...
def bkde2D(x: np.ndarray[Any, np.dtype[np.float64]], bandwidth: float | np.ndarray[Any, np.dtype[np.float64]], gridsize: tuple[int, int] | list[int] | np.ndarray[Any, np.dtype[np.int64]] = (51, 51), range_x: tuple[tuple[float, float], tuple[float, float]] | list[list[float]] | np.ndarray[Any, np.dtype[np.float64]] | None = None, truncate: bool = True) -> dict[str, np.ndarray[Any, np.dtype[np.float64]]]:
    ...
    for i in range(2):
        L[i] = min(int(np.floor(tau * h[i] * (M[i] - 1) / (b[i] - a[i]))), int(M[i]) - 1)
        lvec = np.arange(0, L[i] + 1)
        fac = (b[i] - a[i]) / (h[i] * (M[i] - 1))
        z = (np.exp(-((lvec * fac) ** 2) / 2) / np.sqrt(2 * np.pi)) / h[i]
    ...
    ## R's fft(z, inverse = TRUE) is the unnormalized inverse transform, i.e.
    ## (P1*P2) * np.fft.ifft2(z). The R code divides by (P1*P2) afterwards, which
    ## exactly cancels numpy's normalization, so np.fft.ifft2(z) can be used directly.
...
def bkfe(x: np.ndarray[Any, np.dtype[np.float64]], drv: int, bandwidth: float | None, gridsize: int = 401, range_x: tuple[float, float] | list[float] | np.ndarray[Any, np.dtype[np.float64]] | None = None, binned: bool = False, truncate: bool = True) -> float:
    # Install safeguard against non-positive bandwidths:
    if bandwidth is not None and bandwidth <= 0:
        raise ValueError("'bandwidth' must be strictly positive")
    ...
    kappam = np.exp(-arg**2 / 2) / np.sqrt(2 * np.pi)
    ...
    return float(np.sum(gcounts * (np.real(conv)[0:M])) / (n ** 2))
...
def blkest(x: np.ndarray[Any, np.dtype[np.float64]], y: np.ndarray[Any, np.dtype[np.float64]], Nval: int, q: int) -> dict[str, float]:
    ...
    # Reimplementation of the Fortran subroutine blkest (src/blkest.f)
    qq = q + 1
    RSS = 0.0
    th22e = 0.0
    th24e = 0.0
    ...
    for j in range(1, Nval + 1):
        ...
        # Set up "X" matrix: column k (0-based) holds xj**k.
        Xmat = np.zeros((nj, qq), dtype=np.float64)
        Xmat[:, 0] = 1.0
        for k in range(2, qq + 1):
            Xmat[:, k - 1] = xj ** (k - 1)

        # Least-squares solve via QR decomposition, mirroring the
        # Fortran calls to dqrdc/dqrsl.
        Qmat, Rmat = np.linalg.qr(Xmat)
        coef = np.linalg.solve(Rmat, Qmat.T @ yj)
...
def cpblock(X: np.ndarray[Any, np.dtype[np.float64]], Y: np.ndarray[Any, np.dtype[np.float64]], Nmax: int, q: int) -> int:
    ...
    #
    # Note on the return value: R's `order(Cpvec)[1L]` returns the
    # 1-based position of the minimum C_p value within Cpvec, where
    # Cpvec[Nval] corresponds to the fit using Nval blocks (Nval running
    # from 1 to Nmax). That 1-based position therefore already equals
    # the optimal number of blocks Nval, which is what callers such as
    # dpill use directly as a block count (e.g. blkest(x, y, Nval, 4)).
    # This function returns that same integer block count N (1..Nmax),
    # not a 0-based array index, to preserve the original semantics.
    n = len(X)

    # Sort the (X, Y) data with respect to the X's.
    order_idx = np.argsort(np.asarray(X, dtype=np.float64), kind="mergesort")
    X = np.asarray(X, dtype=np.float64)[order_idx]
    Y = np.asarray(Y, dtype=np.float64)[order_idx]

    # Reimplementation of the Fortran subroutine cp (src/cp.f).
    # 'q' itself is unused beyond computing qq, matching the R code's
    # comment "remove unused 'q' 2007-07-10".
    qq = q + 1
    ...
    # order(Cpvec)[1L] in R: the (1-based) position of the smallest
    # C_p value, using a stable sort so ties resolve to the first
    # (smallest) index -- matching np.argmin's tie-breaking behavior.
...
def dpill(x: np.ndarray[Any, np.dtype[np.float64]], y: np.ndarray[Any, np.dtype[np.float64]], blockmax: int = 5, divisor: int = 20, trim: float = 0.01, proptrun: float = 0.05, gridsize: int = 401, range_x: tuple[float, float] | list[float] | np.ndarray[Any, np.dtype[np.float64]] | None = None, truncate: bool = True) -> float:
    ...
    n0 = len(x)
    indlow = int(math.floor(trim * n0))
    indupp = n0 - int(math.floor(trim * n0))
...
def linbin(X: np.ndarray[Any, np.dtype[np.float64]], gpoints: np.ndarray[Any, np.dtype[np.float64]], truncate: bool = True) -> np.ndarray[Any, np.dtype[np.float64]]:
    ...
    # Reimplementation of the Fortran subroutine linbin (src/linbin.f)
    gcnts = np.zeros(M, dtype=np.float64)
    delta = (b - a) / (M - 1)
    for i in range(n):
        lxi = ((X[i] - a) / delta) + 1

        # Find integer part of "lxi" (truncation toward zero, matching Fortran int())
        li = int(lxi)
...
def linbin2D(X: np.ndarray[Any, np.dtype[np.float64]], gpoints1: np.ndarray[Any, np.dtype[np.float64]], gpoints2: np.ndarray[Any, np.dtype[np.float64]]) -> np.ndarray[Any, np.dtype[np.float64]]:
    ...
    # contained in "gpoints1"/"gpoints2" using the linear
    # binning strategy.
    n = X.shape[0]
    x1 = X[:, 0]
    x2 = X[:, 1]
    ...
    # Reimplementation of the Fortran subroutine lbtwod (src/linbin2D.f)
    # gcnts is built directly as an (M1, M2) array so that gcnts[row, col]
    # matches R's matrix(out[[9L]], M1, M2), which reshapes the flat
    # Fortran output in column-major order.
    ...
    for i in range(n):
        ...
        # Find integer part of "lxi1" and "lxi2" (truncation toward zero,
        # matching Fortran int())
        li1 = int(lxi1)
        li2 = int(lxi2)
...
def locpoly(x: np.ndarray[Any, np.dtype[np.float64]], y: np.ndarray[Any, np.dtype[np.float64]] | None = None, drv: int = 0, degree: int | None = None, kernel: str = "normal", bandwidth: float | np.ndarray[Any, np.dtype[np.float64]] | None = None, gridsize: int = 401, bwdisc: int = 25, range_x: tuple[float, float] | list[float] | np.ndarray[Any, np.dtype[np.float64]] | None = None, binned: bool = False, truncate: bool = True) -> dict[str, np.ndarray[Any, np.dtype[np.float64]]]:
    ...
    # NOTE: The following block is a faithful translation of the FORTRAN
    # subroutine "locpol" (src/locpoly.f). To preserve the original
    # 1-based indexing logic of the Fortran/R code exactly, all arrays
    # below are allocated with one extra (unused) leading slot at index 0,
    # so that index "k" in this block corresponds directly to index "k"
    # in the original Fortran source (k = 1, ..., M).
    xcnts = np.zeros(M + 1, dtype=np.float64)
    xcnts[1:M + 1] = xcounts
    ycnts = np.zeros(M + 1, dtype=np.float64)
    ycnts[1:M + 1] = ycounts
    indic1 = np.zeros(M + 1, dtype=np.int64)
    indic1[1:M + 1] = indic
    Lvec1 = np.zeros(Q + 1, dtype=np.int64)
    Lvec1[1:Q + 1] = Lvec
    hdisc1 = np.zeros(Q + 1, dtype=np.float64)
    hdisc1[1:Q + 1] = hdisc
    ...
    curvest = math.factorial(drv) * cvest[1:M + 1]
...
def onUnload(libpath: str) -> None:
    # No-op: exists only for structural parity with the original R
    # .onUnload package-unload hook (library.dynam.unload("KernSmooth", libpath)).
    # The Python translation has no compiled shared library to unload.
    pass
...
def rlbin(X: np.ndarray[Any, np.dtype[np.float64]], Y: np.ndarray[Any, np.dtype[np.float64]], gpoints: np.ndarray[Any, np.dtype[np.float64]], truncate: bool = True) -> dict[str, np.ndarray[Any, np.dtype[np.float64]]]:
    ...
    # Reimplementation of the Fortran subroutine rlbin (src/rlbin.f)
...
def sdiag(x: np.ndarray[Any, np.dtype[np.float64]], drv: int = 0, degree: int = 1, kernel: str = "normal", bandwidth: float | np.ndarray[Any, np.dtype[np.float64]] | None = None, gridsize: int = 401, bwdisc: int = 25, range_x: tuple[float, float] | list[float] | np.ndarray[Any, np.dtype[np.float64]] | None = None, binned: bool = False, truncate: bool = True) -> dict[str, np.ndarray[Any, np.dtype[np.float64]]]:
    ...
    # NOTE: The following block is a faithful translation of the FORTRAN
    # subroutine "sdiag" (src/sdiag.f). To preserve the original 1-based
    # indexing logic of the Fortran/R code exactly, all arrays below are
    # allocated with one extra (unused) leading slot at index 0, so that
    # index "k" in this block corresponds directly to index "k" in the
    # original Fortran source (k = 1, ..., M). Unlike "locpol" (which
    # solves Smat * Tvec = Tvec for the fitted value), "sdiag" instead
    # inverts Smat and extracts its (1,1) entry (0-based: [0, 0]) as the
    # diagonal entry of the smoother/hat matrix at each grid point, since
    # no response data (y/ycounts) is involved.
    xcnts = np.zeros(M + 1, dtype=np.float64)
    xcnts[1:M + 1] = xcounts
    indic1 = np.zeros(M + 1, dtype=np.int64)
    indic1[1:M + 1] = indic
...
def sstdiag(x: np.ndarray[Any, np.dtype[np.float64]], drv: int = 0, degree: int = 1, kernel: str = "normal", bandwidth: float | np.ndarray[Any, np.dtype[np.float64]] | None = None, gridsize: int = 401, bwdisc: int = 25, range_x: tuple[float, float] | list[float] | np.ndarray[Any, np.dtype[np.float64]] | None = None, binned: bool = False, truncate: bool = True) -> dict[str, np.ndarray[Any, np.dtype[np.float64]]]:
    ...
    # NOTE: The following block is a faithful translation of the FORTRAN
    # subroutine "sstdg" (src/sstdiag.f). To preserve the original 1-based
    # indexing logic of the Fortran/R code exactly, all arrays below are
    # allocated with one extra (unused) leading slot at index 0, so that
    # index "k" in this block corresponds directly to index "k" in the
    # original Fortran source (k = 1, ..., M). "sstdiag" is structurally
    # very similar to "sdiag", but in addition to the "ss"/"Smat" moment
    # matrices built from the kernel weights it also accumulates a second
    # set of moments "uu"/"Umat" built from the *squared* kernel weights
    # (since SS^T involves squared smoother weights). The diagonal entry
    # of SS^T at each grid point k is then formed as
    # SSTd[k] = sum_{i,j} Smat^{-1}[0,i] * Umat[i,j] * Smat^{-1}[j,0]
    # i.e. the (1,1) (1-based)/(0,0) (0-based) entry of
    # Smat^{-1} @ Umat @ Smat^{-1}.
    xcnts = np.zeros(M + 1, dtype=np.float64)
    xcnts[1:M + 1] = xcounts
    indic1 = np.zeros(M + 1, dtype=np.int64)
            ...
            for i in range(1, Q + 1):
                ...
                for j in range(lo, hi + 1):
                    if indic1[j] == i:
                        fac = 1.0
                        w = fkap[k - j + midpts[i]]
                        ss[j, 1] = ss[j, 1] + xcnts[k] * w
                        uu[j, 1] = uu[j, 1] + xcnts[k] * (w ** 2)
                        for ii in range(2, ppp + 1):
                            fac = fac * delta * (k - j)
                            ss[j, ii] = ss[j, ii] + xcnts[k] * w * fac
                            uu[j, ii] = uu[j, ii] + xcnts[k] * (w ** 2) * fac
    ...
    for k in range(1, M + 1):
        ...
        for i in range(pp):
            for j in range(pp):
                sstd += Smat_inv[0, i] * Umat[i, j] * Smat_inv[j, 0]
        SSTd[k] = sstd
