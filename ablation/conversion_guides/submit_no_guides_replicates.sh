#!/bin/bash
#$ -N ablation_no_guides
#$ -t 1-10
#$ -cwd
#$ -j y
#$ -o ablation/logs/no_guides_$TASK_ID.log
#$ -q long
...
#   qsub ablation/conversion_guides/submit_no_guides_replicates.sh
#
...
OUT_DIR="${REPO_ROOT}/ablation/conversion_guides/no_guides/${REPLICATE}"
...
apptainer exec --no-mount cwd --pwd "${REPO_ROOT}" "${BINDS[@]}" "${IMAGE}" \
  claude -p "/convert-r-file-to-python KernSmooth/R/all.R structural_analysis/R/all.json structural_analysis/dependency_levels.csv \"You are forbidden to use any conversion guides. Skip all steps that require conversion guides, and tell all the sub-agents to do the same.\" ${OUT_DIR}" \
