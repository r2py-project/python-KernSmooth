...
def _on_unload(libpath: str) -> None:
    # Python does not provide a direct analogue to R's library.dynam.unload,
    # which explicitly unloads a package's compiled shared library from the
    # running process. Python's C extension modules are not unloaded this
    # way, so this hook is retained only for structural parity with the
    # original R package and performs no action.
    return None
...
def linbin(X: np.ndarray[Any, np.dtype[np.float64]], gpoints: np.ndarray[Any, np.dtype[np.float64]], truncate: bool = True) -> np.ndarray[Any, np.dtype[np.float64]]:
    ...
    _KernSmooth.linbin(x_arr, n, float(a), float(b), M, trun, gcnts)
    return gcnts
...
def linbin2D(X: np.ndarray[Any, np.dtype[np.float64]], gpoints1: np.ndarray[Any, np.dtype[np.float64]], gpoints2: np.ndarray[Any, np.dtype[np.float64]]) -> np.ndarray[Any, np.dtype[np.float64]]:
    n = X.shape[0]
    x_arr = np.concatenate([np.asarray(X[:, 0], dtype=np.float64),
    ...
    _KernSmooth.lbtwod(x_arr, n, float(a1), float(a2), float(b1), float(b2),
                        M1, M2, gcnts)
    return np.reshape(gcnts, (M1, M2), order='F')
...
def rlbin(X: np.ndarray[Any, np.dtype[np.float64]], Y: np.ndarray[Any, np.dtype[np.float64]], gpoints: np.ndarray[Any, np.dtype[np.float64]], truncate: bool = True) -> dict[str, np.ndarray[Any, np.dtype[np.float64]]]:
    ...
    _KernSmooth.rlbin(x_arr, y_arr, n, float(a), float(b), M, trun, xcounts, ycounts)
...
def cpblock(X: np.ndarray[Any, np.dtype[np.float64]], Y: np.ndarray[Any, np.dtype[np.float64]], Nmax: int, q: int) -> int:
    ...
    # order(Cpvec)[1L]: 1-based index of the minimum value, preserving R's
    # (stable) tie-breaking convention
    return int(np.argsort(Cpvec, kind="stable")[0]) + 1
...
def blkest(x: np.ndarray[Any, np.dtype[np.float64]], y: np.ndarray[Any, np.dtype[np.float64]], Nval: int, q: int) -> dict[str, float]:
    ...
    _KernSmooth.blkest(x_sorted, y_sorted, n, q, qq, Nval, xj, yj,
                        coef, Xmat, wk, qraux, sigsqe, th22e, th24e)
...
def bkfe(x: np.ndarray[Any, np.dtype[np.float64]], drv: int, bandwidth: float, gridsize: int = 401, range_x: tuple[float, float] | None = None, binned: bool = False, truncate: bool = True) -> float:
    ...
    # P * numpy's (normalized) ifft; the subsequent division by P in the R code
    # therefore reduces exactly to numpy's np.fft.ifft.
    conv = np.fft.ifft(kappam * Gcounts).real
...
def bkde(x: np.ndarray[Any, np.dtype[np.float64]], kernel: str = "normal", canonical: bool = False, bandwidth: float | None = None, gridsize: int = 401, range_x: tuple[float, float] | None = None, truncate: bool = True) -> dict[str, np.ndarray[Any, np.dtype[np.float64]]]:
    ...
    if kernel not in valid_kernels:
        raise ValueError(f"'kernel' should be one of {valid_kernels}")
...
def bkde2D(x: np.ndarray[Any, np.dtype[np.float64]], bandwidth: float | Sequence[float] | np.ndarray[Any, np.dtype[np.float64]], gridsize: Sequence[int] | np.ndarray[Any, np.dtype[np.int64]] = (51, 51), range_x: Sequence[Sequence[float]] | None = None, truncate: bool = True) -> dict[str, np.ndarray[Any, np.dtype[np.float64]]]:
    # Install safeguard against non-positive bandwidths:
    ...
    # Rename common variables
    n = x.shape[0]
    M = np.asarray(gridsize, dtype=np.int64).copy()
    ...
    if range_x is None:
        ...
        for id_ in range(2):
            range_x_list[id_] = (float(np.min(x[:, id_]) - 1.5 * h[id_]),
    ...
    rp_result = np.real(np.fft.ifft2(rp_fft * sp_fft))[0:M1, 0:M2]
    ...
    rp_result = rp_result * (rp_result > 0).astype(np.float64)
...
def dpih(x: np.ndarray[Any, np.dtype[np.float64]], scalest: str = "minim", level: int = 2, gridsize: int = 401, range_x: tuple[float, float] | None = None, truncate: bool = True) -> float:
    ...
    if scalest_name == "stdev":
        ...
        scalest_val = min((np.quantile(x, 0.75) - np.quantile(x, 0.25)) / 1.349, np.sqrt(np.var(x, ddof=1)))
...
def dpik(x: np.ndarray[Any, np.dtype[np.float64]], scalest: str = "minim", level: int = 2, kernel: str = "normal", canonical: bool = False, gridsize: int = 401, range_x: tuple[float, float] | None = None, truncate: bool = True) -> float:
    ...
    if scalest not in valid_scalest:
        raise ValueError("'scalest' should be one of " + ", ".join(valid_scalest))

    if scalest == "stdev":
        scalest = np.sqrt(np.var(x, ddof=1))
    elif scalest == "iqr":
        scalest = (np.quantile(x, 3 / 4) - np.quantile(x, 1 / 4)) / 1.349
    else:  # scalest == "minim"
        scalest = min(
    ...
    # Replace input data by standardised data for numerical stability:
    x_mean = np.mean(x)
    sx = (np.asarray(x, dtype=np.float64) - x_mean) / scalest
    sa = (a - x_mean) / scalest
    sb = (b - x_mean) / scalest
    ...
    return scalest * del0 * (1 / (psi4hat * n)) ** (1 / 5)
...
def locpoly(x: np.ndarray[Any, np.dtype[np.float64]], y: np.ndarray[Any, np.dtype[np.float64]] | None = None, drv: int = 0, degree: int | None = None, kernel: str = "normal", *, bandwidth: float | np.ndarray[Any, np.dtype[np.float64]], gridsize: int = 401, bwdisc: int = 25, range_x: tuple[float, float] | None = None, binned: bool = False, truncate: bool = True) -> dict[str, np.ndarray[Any, np.dtype[np.float64]]]:
    ...
    if len(bandwidth_arr) == M:
        ...
        indic = np.ones(M, dtype=np.int32)
        Q = 1
        Lvec = np.full(Q, int(np.floor(tau * bandwidth_arr[0] / delta)), dtype=np.int32)
    ...
    _KernSmooth.locpol(
        xcounts_arr, ycounts_arr, drv, delta, hdisc, Lvec, indic, midpts,
...
def sdiag(x: np.ndarray[Any, np.dtype[np.float64]], bandwidth: float | np.ndarray[Any, np.dtype[np.float64]], drv: int = 0, degree: int = 1, kernel: str = "normal", gridsize: int = 401, bwdisc: int = 25, range_x: tuple[float, float] | None = None, binned: bool = False, truncate: bool = True) -> dict[str, np.ndarray[Any, np.dtype[np.float64]]]:
    ...
    _KernSmooth.sdiag(np.asarray(xcounts, dtype=np.float64), float(delta),
                       np.asarray(hdisc, dtype=np.float64), np.asarray(Lvec, dtype=np.int64),
                       np.asarray(indic, dtype=np.int64), midpts, M, Q, fkap, pp, ppp,
                       ss, Smat, work, det, ipvt, Sdg)

    return {"x": gpoints, "y": Sdg}


def sstdiag(x: np.ndarray[Any, np.dtype[np.float64]], bandwidth: float | np.ndarray[Any, np.dtype[np.float64]], drv: int = 0, degree: int = 1, kernel: str = "normal", gridsize: int = 401, bwdisc: int = 25, range_x: tuple[float, float] | None = None, binned: bool = False, truncate: bool = True) -> dict[str, np.ndarray[Any, np.dtype[np.float64]]]:
    if range_x is None and not binned:
        range_x = (float(np.min(x)), float(np.max(x)))
    ...
    _KernSmooth.sstdg(np.asarray(xcounts, dtype=np.float64), float(delta),
                       np.asarray(hdisc, dtype=np.float64), np.asarray(Lvec, dtype=np.int64),
                       np.asarray(indic, dtype=np.int64), midpts, M, Q, fkap, pp, ppp,
                       ss, uu, Smat, Umat, work, det, ipvt, SSTd)
