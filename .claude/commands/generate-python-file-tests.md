...
* Navigate to the documentation of the original R package (e.g., via CRAN, local `.Rd` files, or the package's reference manual). *Example:* For the `KernSmooth` package, review its documentation at `https://cran.r-project.org/web/packages/KernSmooth/refman/KernSmooth.html`. Use this to identify all the public interfaces of the R package.
