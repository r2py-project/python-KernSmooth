...
- Pass the following explicit context to the agent for each invocation:
  ...
  3. The path to the corresponding Python library folder (e.g., the root of the `r2py_kernsmooth` project, ensuring the agent can correctly locate or create the `tests/` directory as per its instructions).
